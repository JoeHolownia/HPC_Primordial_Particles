...
void::IOParser::WriteStateToOutFile(Particle* state, int n) {
    ...
    double* arr_colour = new double[n];

    // fill temp arrays from particle structs in state
    for (int i = 0; i < n; i++) {
        arr_x[i] = state[i].x;
        arr_y[i] = state[i].y;
        arr_colour[i] = (double)state[i].colour;
    }

    // write coords, and colours
    io_out_disp_file.write((char *)arr_x, sizeof(double) * n);
    io_out_disp_file.write((char *)arr_y, sizeof(double) * n);
    io_out_disp_file.write((char *)arr_colour, sizeof(double) * n);
